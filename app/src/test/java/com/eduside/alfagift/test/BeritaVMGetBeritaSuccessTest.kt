...
class BeritaVMGetBeritaSuccessTest : BaseTest() {
    ...
    @Test
    fun `Ketika request getBerita(), response success dan data tepat`() {
        runBlocking {
            viewModel.getSyncBerita()
            val request = server.takeRequest()
            Truth.assertThat(request.requestLine).isEqualTo("GET /everything?q=Apple&from=2023-04-10&sortBy=popularity HTTP/1.1")
            Truth.assertThat(viewModel.getBeritaResponse.getOrAwaitValue().status)
                .isEqualTo("ok")
            Truth.assertThat(viewModel.getBeritaResponse.getOrAwaitValue().totalResults)
                .isEqualTo(43986)
